...
class Recommend extends React.Component {
    ...
    render() {
        return (
            <section className="recommend" ref="list" >
    ...
    loding() {
        ...
        .then(data => {
            ...
            fetch(api.highquality())
            .then(response => response.json())
            .then(data => {
                self.setState({
                recomList:data.playlists
              })
            });
        });
        
    }
    getMessage(JsonCallback) {}
    componentDidMount() {
        this.loding()
    }
};
...
const mapdispatchtoprops=(dispatch)=>{
    return {
        openRecommend (msg) {
            dispatch({ type: 'OPEN_RECOMMEND',msg})
        }
    }
}
